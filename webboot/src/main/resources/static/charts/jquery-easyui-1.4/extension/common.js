...
//地址格式化
	function addrProvice(provice,city,county,street,detail){
		$('#'+provice).combobox({//加载省下拉菜单
			...
			$('#' + provice).combobox('textbox').focus(function(){
			$('#' + provice).combobox('showPanel');
			});
	}
	...
	function filterAreaByUC(cityCode) {
		var getArea;
		postAjax(false, getContextPath() +"/crm/param/getCitiesByParentCode?parentCode="+cityCode, function(data) {getArea = data}, 'json');
		return getArea;
	}
	//地址格式化
	function addrProviceByUC(provice,city,county,street,detail){
		$('#'+provice).combobox({//加载省下拉菜单
			data:getProvinces,
			valueField : 'cityNo',
			textField : 'cityName',
			validType : 'selectValidator["'+provice+'"]',
			filter : function(q, row) {
				if (row.cityName.indexOf(q) != -1) {
					return true;
				}
			} ,
			onSelect:function(record){
				var getcity = filterAreaByUC( record.cityNo);
				$('#'+city).combobox('showPanel');
				$('#'+city).combobox({//加载市下拉菜单
				    data:getcity,
				    valueField:'cityNo',
					...
					onSelect:function(record){
						var getcounty = filterAreaByUC( record.cityNo);
						$('#'+county).combobox('showPanel');
						$('#'+county).combobox({//加载省下拉菜单
							data:getcounty,
							valueField:'cityNo',
