 ...
 * author duanshaowei
 */
var sessionStorageGrid = {
		/**
		 * 此方法用于datagrid onBeforeLoad事件
		 * onBeforeLoad: function(param){
	        	sessionStorageGrid.storageEntry(this, param, callback);
	        },
		 * 
		 * @param gridObj datagrid对象
		 * @param pararm 参数
		 * @param callback 回调函数用于扩展
		 */
		storageEntry : function (gridObj,pararm,callback,crud_listUrl){
			//var url = crud_listUrl;
			var url = location.href;
			if(url.indexOf("?")>0){
				url=url.substring(0,url.indexOf("?"));	
			}
			
			if(typeof(pararm) != "undefined" && this.isLoadType(url)){
				var obj = this.getParam(url);
				if(null != obj){
					for(var property in obj){
						var value = obj[property];
						if(value != undefined){
							pararm[property] = value;
							if (property == 'page'){
								$(gridObj).datagrid("getPager").pagination("options").pageNumber=value;
								$(gridObj).datagrid("getPager").pagination("options").pageSize=obj['rows'];
				        	  } 
						}
					}
				}
			}
			this.addParam(pararm, url);
			 $(gridObj).datagrid('options').queryParams = pararm;
			var isFunc = $.isFunction(callback);
			if(isFunc && typeof(isFunc) != 'undefined'){
				callback();
			}
		},
		/**
		 * 跳转后是否加载sessionStorage中url参数 列表页面点击使用
		 * @param url
		 * @param callback
		 */
		isListLoadStorageParam : function(callback){
			var url=location.href;
			if(typeof(url)=="string" && window.sessionStorage){
				if(url.indexOf("?")>0){
					url=url.substring(0,url.indexOf("?"));	
				}
				var _storage = window.sessionStorage ;
				url = url + "_isLoad";
				_storage.setItem(url,true);
			}
			var isFunc = $.isFunction(callback);
			if(isFunc && typeof(isFunc) != 'undefined'){
				callback();
		...
		/**
		 * 跳转后是否加载sessionStorage中url参数  由列表页面点击进入如修改，查看页面使用
		 * @param url
		 * @param callback
		 */
		isLoadStorageParam : function(callback){
		 ...
		 * 判断当前url类型
		 * @param url
		 * @returns type ture/false 加载/不加载
		 */
		isLoadType : function(url){
			if(typeof(url)=="string" && window.sessionStorage){
				url = url + "_isLoad";
				var isLoad = window.sessionStorage.getItem(url);
				if(isLoad == 'true')
					return true;
			}
			return false;
		...
		/**
		 * param添加到sessionStorage当中
		 * @param pararm
		 * @param url
		 */
		addParam : function(pararm,url){
			var tojson_param = JSON.stringify(pararm);
			if(typeof(tojson_param)!="undefined" && window.sessionStorage){
				var _storage = window.sessionStorage ;
				_storage.setItem(url,tojson_param);
				_storage.setItem(url+"_isLoad",false);
			}
		},
		/**
		 * 获取sessionStorage对象url对应的value
		 * @param url
		 * @returns
		 */
		getParam : function(url){
			if(typeof(url)=="string" && window.sessionStorage){
				var _storage = window.sessionStorage ;
				var _json_object = _storage.getItem(url);
				var obj = $.parseJSON(_json_object);
...
(function($){
	function buildGrid(target, options){
	    ...
	    var crud_onBeforeLoad = opts.crud_onBeforeLoad;
		...
		$(target).datagrid({
	        ...
	        onBeforeLoad: function(param){
	        	sessionStorageGrid.storageEntry(this, param, crud_onBeforeLoad,crud_listUrl);
	        },
		...
		/*if(typeof($(target).datagrid("getPager"))!="undefined"){
					...
					$.messager.confirm('警告', '确认删除吗？',function(a){
			     		if(a)$.post(opts.crud_deleteUrl,{"id":delId},function(data){
			     				...
			     				if(data.result.success){
			     					sessionStorageGrid.isListLoadStorageParam();
			     					dg.datagrid('reload');
					   			} else {
					   				$.messager.show({
										title:'警告',
